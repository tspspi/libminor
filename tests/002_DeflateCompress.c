...
static uint8_t* lpMemSource_Offset = NULL;
static unsigned long int dwMemSource_Length = 0;
static unsigned long int dwMemSource_Pos = 0;

static enum minorError streamSource_Mem_Read(
	...
	if((dwMemSource_Length - dwMemSource_Pos) == 0) { return minorE_EndOfStream; }

	if(lpBytesRead == NULL) {
		if((dwMemSource_Length - dwMemSource_Pos) < dwBytesToRead) { return minorE_InvalidParam; }
	} else {
		(*lpBytesRead) = 0;
	}

	i = 0;
	while((dwBytesToRead > 0) && ((dwMemSource_Length - dwMemSource_Pos) > 0)) {
		lpDestinationBuffer[i] = lpMemSource_Offset[dwMemSource_Pos];
		dwMemSource_Pos = dwMemSource_Pos + 1;
		if(lpBytesRead != NULL) { (*lpBytesRead) = (*lpBytesRead) + 1; }
	}

	return minorE_Ok;
}
static struct minorStreamSource streamSourceMem = {
	&streamSource_Mem_Read
};
...
int main(int argc, char* argv[]) {
	...
	e = minorCompressorCreate_Deflate(&lpCompressor, minorAlgorithm_Deflate, &sysApi, NULL);
	if(e != minorE_Ok) {
	...
	e = minorCompressorAttachSource_Deflate(lpCompressor, &streamSourceFile);
	...
	e = minorCompressorAttachSink_Deflate(lpCompressor, &streamSinkFile);
	if(e != minorE_Ok) { printf("%s:%u failed (code %u)\n", __FILE__, __LINE__, e); return -1; } else { printf("ok\n"); }
	...
	/* Open output file */
	fTestOut = fopen("./1.txt.rawdeflate", "w+b");
	if(fTestOut == NULL) {
		printf("%s:%u Failed to open file\n", __FILE__, __LINE__);
		return -1;
	}
	...
	e = minorCompressor_TransferFromInput_Deflate(lpCompressor, fileSize, &dwBytesDone);
	...
	e = minorCompressorRelease_Deflate(lpCompressor);
	if(e != minorE_Ok) {
		printf("%s:%u failed (code %u)\n", __FILE__, __LINE__, e);
		return -1;
	}
	...
	fTestIn = fopen("./1.txt.rawdeflate", "rb");
	if(fTestIn == NULL) {
		printf("%s:%u Failed to open rawdeflate file\n", __FILE__, __LINE__);
		return -1;
	}
	fTestOut = fopen("./1.txt.rawdeflatedec", "w+b");
	if(fTestOut == NULL) {
		printf("%s:%u Failed to open file\n", __FILE__, __LINE__);
		return -1;
	}
	...
	printf("%s:%u Attaching source ... ", __FILE__, __LINE__);
	e = minorDecompressorAttachSource_Deflate(lpDecompressor, &streamSourceFile);
	if(e != minorE_Ok) { printf("%s:%u failed (code %u)\n", __FILE__, __LINE__, e); return -1; } else { printf("ok\n"); }
	printf("%s:%u Attaching sink ... ", __FILE__, __LINE__);
	e = minorDecompressorAttachSink_Deflate(lpDecompressor, &streamSinkFile);
	if(e != minorE_Ok) { printf("%s:%u failed (code %u)\n", __FILE__, __LINE__, e); return -1; } else { printf("ok\n"); }

	printf("%s:%u Transferring %lu test bytes into the decompressor ", __FILE__, __LINE__, fileSize);
	e = minorDecompressor_TransferFromInput_Deflate(lpDecompressor, fileSize, &dwBytesDone);
	if((e != minorE_Ok) && (e != minorE_EndOfStream) && (e != minorE_Finished)) {
		printf("%s:%u Failed. Error Code %u\n", __FILE__, __LINE__, e);
	} else {
	...
	e = minorDecompressorRelease_Deflate(lpDecompressor);
	if(e != minorE_Ok) {
		printf("%s:%u failed (code %u)\n", __FILE__, __LINE__, e);
		return -1;
	}
